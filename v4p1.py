...
def create_vector_database_and_store_in_cosmos(judgments, endpoint, api_key, cosmos_endpoint, cosmos_key, database_name, container_name):
    splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=100)
    
    # Modify this to create a list of Document objects from the judgments
    documents = [Document(page_content=judgment) for judgment in judgments]
    
    # Debugging: Print the first judgment to verify content
    if documents:
        print(f"First document: {documents[0].page_content[:500]}...")  # Print the first 500 characters
    else:
    ...
    for i, (embedding, document) in enumerate(zip(embeddings, split_docs)):
        # Store each embedding along with its corresponding content in Cosmos DB
        item = {
            "id": str(i),  # Unique ID for each item
            "embedding": embedding,
            "content": document.page_content  # Storing the 'page_content' of each split document
        }
        container.upsert_item(item)  # Store or update the item
        # Debugging: Print info on each item being upserted
...
if judgments:
    create_vector_database_and_store_in_cosmos(judgments, embedding_endpoint, api_key, cosmos_endpoint, cosmos_key, database_name, cosmos_container_name)
else:
